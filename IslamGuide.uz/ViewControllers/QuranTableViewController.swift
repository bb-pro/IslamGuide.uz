    ...
    func checkPreferredLanguage() {
        if let preferredLanguages = UserDefaults.standard.array(forKey: "AppleLanguages") as? [String],
           let preferredLanguage = preferredLanguages.first {
            if preferredLanguage.lowercased().hasPrefix("uz") {
                fetchTranslation(source: "uz.sodik")
            } else if preferredLanguage.lowercased().hasPrefix("ru") {
                fetchTranslation(source: "ru.kuliev")
            } else {
                fetchTranslation(source: "en.asad")
            }
        }
    }
        
        func fetchTranslation(source: String) {
            networkManager.getData(for: source) { [weak self] result in
                switch result {
